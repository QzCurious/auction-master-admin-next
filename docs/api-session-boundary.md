...
- `api/transport.ts`: framework-independent HTTP execution and safe error decoding. The shared instance/configuration in `server/` never retains user credentials.
...
- Error meanings (`unauthenticated`, `expired`, `forbidden`, `validation`, `service`) are framework-independent. The existing error-envelope/toast/redirect interface is retained at the adapter. Service failures cannot turn into login redirects just because an upstream body contains code `1003`. Full query failure semantics, permission evaluation, invalidation inventory, and polling behavior remain PR 2/3 work.
