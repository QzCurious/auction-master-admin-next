...
function generateGraph({ flow, startId, getMessage, getValue }: StatusFlowGraphProps): GraphData {
          ...
          label: (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
              <div style={{ fontWeight: 'bold' }}>
                {getMessage(id)} <span>({getValue(id)})</span>
              </div>
              {
...
interface StatusFlowGraphProps {
  flow: Record<string, Step>;
  startId: string;
  getMessage: (status: string) => string;
  getValue: (status: string) => number;
}

export function FlowGraph(props: StatusFlowGraphProps) {
  const [highlightedEdges, setHighlightedEdges] = useState<Set<string>>(new Set());

  const { nodes, edges } = useMemo(() => generateGraph(props), [props]);
