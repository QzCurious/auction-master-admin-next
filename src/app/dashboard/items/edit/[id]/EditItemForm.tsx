...
const FormSchema = z
  .object({
    ...
    directPurchasePrice: z.number().min(0, '不可為負數').int('請輸入整數'),
    minEstimatedPrice: z.coerce.number().min(0, '不可為負數').int('請輸入整數').optional(),
    maxEstimatedPrice: z.coerce.number().min(0, '不可為負數').int('請輸入整數').optional(),
    reservePrice: z.number().min(0, '不可為負數').int('請輸入整數').min(1, '必填'),
    expireAt: z.coerce.date().nullable(),
    warehouseId: z.string(),
    space: z.number().min(0, '不可為負數').int('請輸入整數'),
    shippingCostsWithinJapan: z.number().min(0, '不可為負數').int('請輸入整數'),
    grossWeight: z.number().min(0, '不可為負數').int('請輸入整數'),
    volumetricWeight: z.number().min(0, '不可為負數').int('請輸入整數'),
  })
